...
void ProjectileSystem::init() {
  GCR.add_listener(
      METHOD_LISTENER(Events::Window::INPUT, ProjectileSystem::input));
  GCR.add_listener(
      METHOD_LISTENER(Events::Game::STOP, ProjectileSystem::reset_bombs));
}

void ProjectileSystem::update(float time) {
  for (auto &entity : entities) {
    if (buttons.test(static_cast<std::size_t>(InputButtons::J)) &&
        entity >= Entities::P_BULLET && entity < Entities::E_GRUNT) {
      ...
      auto &projectile_last_shot =
          GCR.get_component<Projectile>(entity).last_shot;
      if (!bullet_state.active) {
        glm::vec3 player_location =
            GCR.get_component<Transform>(Entities::PLAYER).pos;
        glm::vec3 player_scale =
            GCR.get_component<Sprite>(Entities::PLAYER).scale_factor;
    ...
    if (entity == Entities::P_BOMB) {
      ...
      if (buttons.test(static_cast<std::size_t>(InputButtons::L))) {
        if (bomb_offset > 2)
          continue;                           // can't shoot anymore bombs
