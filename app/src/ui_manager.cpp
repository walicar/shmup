...
void UiManager::show_title() {
  ...
  render_text("press SHIFT to start...", 560.0f, 11.0f, 0.5f,
              glm::vec3(1.0f, 1.0f, 0.0f));
}

void UiManager::show_ui() {
  int hp = GCR.get_component<Hitbox>(Entities::PLAYER).health;
  render_text("Health: " + std::to_string(hp), 210.0f, 570.0f, 0.5f,
              glm::vec3(1.0f, 0.0f, 0.0f));
  render_text("Bombs: " + std::to_string(bombs_left) + "/3", 10.0f, 570.0f,
              0.5f, glm::vec3(0.0f, 1.0f, 0.0f));
  if (boss_mode) {
    int boss_hp = GCR.get_component<Hitbox>(Entities::BOSS).health;
    render_text("Boss Health: " + std::to_string(boss_hp) + "/800", 410.0f,
                570.0f, 0.5f, glm::vec3(1.0f, 0.647f, 0.0f));
  } else {
    render_text("Level: " + std::to_string(checkpoint), 410.0f, 570.0f, 0.5f,
...
UiManager::UiManager(int width, int height, Shader &shader) {
  GCR.add_listener(
      METHOD_LISTENER(Events::Game::BOMB_USED, UiManager::bomb_used));
  GCR.add_listener(
      METHOD_LISTENER(Events::Game::BOSS_TIME, UiManager::boss_time));
  GCR.add_listener(
      METHOD_LISTENER(Events::Game::WAVE_DONE, UiManager::next_wave));
  ...
  for (unsigned char c = 0; c < 128; c++) {
    ...
    glTexStorage2D(GL_TEXTURE_2D, 1, GL_R8, face->glyph->bitmap.width,
                   face->glyph->bitmap.rows);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, face->glyph->bitmap.width,
...
void UiManager::render_text(std::string text, float x, float y, float scale,
                            glm::vec3 color) {
  text_shader->use();
...
void UiManager::bomb_used(Event &e) { bombs_left -= 1; }

void UiManager::boss_time(Event &e) { boss_mode = true; }

void UiManager::next_wave(Event &e) { checkpoint += 1; }
