...
public:
  ...
  void add_listener(EventId eventId,
                    std::function<void(Event &)> const &listener) {
    event_manager->add_listener(eventId, listener);
  }

  void send_event(Event &event) { event_manager->send_event(event); }

  void send_event(EventId eventId) { event_manager->send_event(eventId); }
};
