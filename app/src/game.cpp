...
Game::Game() {
  ...
  for (int i = 0; i < Entities::E_BULLET_AMT; i++) {
    ...
    GCR.add_component(enemy_bullet,
                      Hitbox{.hitbox = glm::vec3(0.5f, 0.5f, 1.0f)});
...
void Game::make_enemy(EnemyType type, int offset) {
  ...
  case GRUNT:
    ...
    GCR.add_component(enemy, Enemy{.type = GRUNT});
    GCR.add_component(enemy,
                      AI{.attack_cooldown = 1.0f + ((offset % 10) * 0.25f),
                         .last_attacked = ((offset % 10) * 0.25f)});
    GCR.add_component(enemy, Hitbox{.hitbox = glm::vec3(0.7f)});
    break;
  case SNIPE:
    GCR.add_component(enemy, SpriteCache::get_sprite("snipe"));
    GCR.add_component(enemy, Enemy{.type = SNIPE});
    GCR.add_component(enemy, AI{.attack_cooldown = 1.0f,
                                .last_attacked = 0.0f + (1.0f * offset)});
    GCR.add_component(enemy, Hitbox{.hitbox = glm::vec3(0.7f)});
    break;
  case HOSE:
    GCR.add_component(enemy, SpriteCache::get_sprite("hose"));
    GCR.add_component(enemy, Enemy{.type = HOSE});
    GCR.add_component(enemy, AI{.attack_cooldown = 0.75f,
                                .last_attacked = 0.0f + (1.25f * offset)});
    GCR.add_component(enemy,
                      Hitbox{.health = HOSE_HP, .hitbox = glm::vec3(0.7f)});
    break;
  case STAR:
    GCR.add_component(enemy, SpriteCache::get_sprite("star"));
    GCR.add_component(enemy, Enemy{.type = STAR});
    GCR.add_component(enemy, AI{.attack_cooldown = 1.0f,
                                .last_attacked = 0.0f + (0.25f * offset)});
    GCR.add_component(enemy,
                      Hitbox{.health = STAR_HP, .hitbox = glm::vec3(0.7f)});
    break;
  case BOSS:
    // manually make boss
    break;
...
void Game::init_player() {
  ...
  GCR.add_component(player, State{.active = true});
  ...
  GCR.add_component(player,
                    Hitbox{.health = PLAYER_HP, .hitbox = glm::vec3(0.15f)});
  ...
  GCR.add_component(player_laser,
                    Transform{.pos = glm::vec3(10.0f, 0.0f, 0.0f)});
  GCR.add_component(player_laser,
                    Hitbox{.hitbox = glm::vec3(0.5f, 1.5f, 1.0f)});
  GCR.add_component(player_laser, Velocity{});
  GCR.add_component(player_laser, Player{});
  GCR.add_component(player_laser, Projectile{.type = LASER, .damage = 2});
  ...
  for (int i = 0; i < Entities::P_BOMB_AMT; i++) {
    ...
    GCR.add_component(player_bomb, Hitbox{.hitbox = glm::vec3(3000.0f)});
  }
  ...
  for (int i = 0; i < Entities::P_BULLET_AMT; i++) {
    ...
    GCR.add_component(player_bullet,
                      Hitbox{.hitbox = glm::vec3(0.5f, 0.5f, 1.0f)});
    GCR.add_component(player_bullet, Velocity{});
    GCR.add_component(player_bullet, Projectile{.damage = 15});
  }
}
...
void Game::init_ecs() {
  ...
  movement_system =
      GCR.register_system<MovementSystem>(); // rename to movement system
  {
    Signature signature;
    signature.set(GCR.get_component_type<Transform>());
